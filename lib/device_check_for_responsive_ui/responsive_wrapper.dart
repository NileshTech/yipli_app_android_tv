...
import 'package:flutter_app/device_check_for_responsive_ui/responsive_utils.dart';
        ...
        ? (widget.backgroundColor ??
            Container(
                color: Theme.of(context)
                    .primaryColor)) // First frame empty background color or default white.
...
class ResponsiveBreakpoint {
  ...
  const ResponsiveBreakpoint.resize(this.breakpoint, this.autoscale,
  ...
  const ResponsiveBreakpoint.autoScale(this.breakpoint, this.autoscale,
      {this.name, this.scaleFactor = 1})
      : this.behavior = ResponsiveBreakpointBehavior.AUTOSCALE,
        assert(breakpoint != null && breakpoint >= 0,
            'Breakpoints cannot be negative. To control behavior from 0, set `default` parameters in `ResponsiveWrapper`.');

  const ResponsiveBreakpoint.autoScaleDown(this.breakpoint, this.autoscale,
      {this.name, this.scaleFactor = 1})
      : this.behavior = ResponsiveBreakpointBehavior.AUTOSCALEDOWN,
        assert(breakpoint != null && breakpoint >= 0,
            'Breakpoints cannot be negative. To control behavior from 0, set `default` parameters in `ResponsiveWrapper`.');

  const ResponsiveBreakpoint.tag(this.breakpoint, this.autoscale,
      {@required this.name})
